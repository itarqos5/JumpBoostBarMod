...
if exist paper\build\libs\*.jar del /q paper\build\libs\*.jar

echo Building Fabric, NeoForge, and Paper...
call gradlew.bat --no-daemon universalModJar :paper:build
if errorlevel 1 exit /b %errorlevel%

if exist dist rmdir /s /q dist
mkdir dist

for %%J in ("build\libs\*.jar" "paper\build\libs\*.jar") do (
